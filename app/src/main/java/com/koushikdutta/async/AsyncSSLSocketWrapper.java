...
public class AsyncSSLSocketWrapper implements AsyncSocketWrapper,
		AsyncSSLSocket {
	...
	static {
		...
		try {
			// critical extension 2.5.29.15 is implemented improperly prior to
			// 4.0.3.
			...
			// fallback is to use a custom SSLContext, and hack around the x509
			// extension.
					...
					public void checkServerTrusted(X509Certificate[] certs,
						...
						for (X509Certificate cert : certs) {
							if (cert != null
								...
								cert.getCriticalExtensionOIDs().remove(
										"2.5.29.15");
						}
					}
				} };
	...
	public static void handshake(AsyncSocket socket, String host, int port,
			SSLEngine sslEngine, TrustManager[] trustManagers,
			HostnameVerifier verifier, boolean clientMode,
			final HandshakeCallback callback) {
		AsyncSSLSocketWrapper wrapper = new AsyncSSLSocketWrapper(socket, host,
				port, sslEngine, trustManagers, verifier, clientMode);
		...
		socket.setClosedCallback(new CompletedCallback() {
			...
			public void onCompleted(Exception ex) {
				if (ex != null)
					...
					callback.onHandshakeCompleted(new SSLException(
							"socket closed during handshake"), null);
			}
		});
		try {
	...
	private AsyncSSLSocketWrapper(AsyncSocket socket, String host, int port,
			SSLEngine sslEngine, TrustManager[] trustManagers,
			HostnameVerifier verifier, boolean clientMode) {
		mSocket = socket;
	...
	private void handleHandshakeStatus(HandshakeStatus status) {
		...
		try {
			if (!finishedHandshake
					&& (engine.getHandshakeStatus() == HandshakeStatus.NOT_HANDSHAKING || engine
				...
				if (clientMode) {
					...
					for (TrustManager tm : trustManagers) {
						try {
							...
							peerCertificates = (X509Certificate[]) engine
								...
								if (hostnameVerifier == null) {
									...
									verifier.verify(
											...
											StrictHostnameVerifier
													.getCNs(peerCertificates[0]),
											StrictHostnameVerifier
													.getDNSSubjectAlts(peerCertificates[0]));
								} else {
									if (!hostnameVerifier.verify(mHost,
											engine.getSession())) {
										throw new SSLException("hostname <"
												+ mHost + "> has been denied");
									}
								}
							}
							trusted = true;
				...
				// handshake can complete during a wrap, so make sure that the
				// call
	...
	public void write(ByteBufferList bb) {
		...
		ByteBuffer writeBuf = ByteBufferList.obtain(calculateAlloc(bb
				.remaining()));
		do {
			...
			try {
				...
				if (res.getStatus() == Status.BUFFER_OVERFLOW) {
					...
					writeBuf = ByteBufferList.obtain(calculateAlloc(bb
							.remaining()));
					handleHandshakeStatus(res.getHandshakeStatus());
				}
			} catch (SSLException e) {
		...
		} while ((remaining != bb.remaining() || (res != null && res
