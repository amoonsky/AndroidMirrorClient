...
public class AsyncServer {
	...
	public static void post(Handler handler, Runnable runnable) {
		...
		ThreadQueue threadQueue = ThreadQueue.getOrCreateThreadQueue(handler
				.getLooper().getThread());
	...
	PriorityQueue<Scheduled> mQueue = new PriorityQueue<Scheduled>(1,
			Scheduler.INSTANCE);
	...
	public AsyncServerSocket listen(final InetAddress host, final int port,
			final ListenCallback handler) {
	...
	private ConnectFuture connectResolvedInetSocketAddress(
			final InetSocketAddress address, final ConnectCallback callback) {
		...
		post(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					ckey = socket.register(mSelector.getSelector(),
	...
	public Cancellable connectSocket(final InetSocketAddress remote,
		...
		lookup.setCallback(new FutureCallback<InetAddress>() {
			...
			public void onCompleted(Exception e, InetAddress result) {
				...
				ret.setComplete(connectResolvedInetSocketAddress(
						new InetSocketAddress(result, remote.getPort()),
						callback));
			}
		});
		return ret;
	...
	public Cancellable connectSocket(final String host, final int port,
			final ConnectCallback callback) {
		return connectSocket(InetSocketAddress.createUnresolved(host, port),
				callback);
	}
	...
	public Future<InetAddress> getByName(String host) {
		return getAllByName(host).then(
				new TransformFuture<InetAddress, InetAddress[]>() {
					@Override
					protected void transform(InetAddress[] result)
							throws Exception {
	...
	public AsyncDatagramSocket connectDatagram(final String host, final int port)
			throws IOException {
		...
		run(new Runnable() {
			...
			public void run() {
				try {
					final SocketAddress remote = new InetSocketAddress(host,
							port);
	...
	public AsyncDatagramSocket openDatagram(final SocketAddress address,
	...
	public AsyncDatagramSocket connectDatagram(final SocketAddress remote)
			throws IOException {
		final DatagramChannel socket = DatagramChannel.open();
		final AsyncDatagramSocket handler = new AsyncDatagramSocket();
		handler.attach(socket);
	...
	private void run(boolean newThread) {
		...
		synchronized (this) {
			if (mSelector != null) {
				...
				try {
					selector = mSelector = new SelectorWrapper(SelectorProvider
		...
		if (reentrant) {
			try {
				...
				try {
					// StreamUtility.closeQuiety is throwing
					// ArrayStoreException?
	...
	private static void run(final AsyncServer server,
			final SelectorWrapper selector, final PriorityQueue<Scheduled> queue) {
		...
		while (true) {
			...
			synchronized (server) {
				if (selector.isOpen()
						&& (selector.keys().size() > 0 || queue.size() > 0))
					continue;

				shutdownEverything(selector);
				if (server.mSelector == selector) {
					server.mQueue = new PriorityQueue<Scheduled>(1,
							Scheduler.INSTANCE);
	...
	private static long lockAndRunQueue(final AsyncServer server,
			final PriorityQueue<Scheduled> queue) {
	...
	private static void runLoop(final AsyncServer server,
			final SelectorWrapper selector, final PriorityQueue<Scheduled> queue)
			throws AsyncSelectorException {
		...
		try {
			synchronized (server) {
				...
				if (readyNow == 0) {
					// if there is nothing to select now, make sure we don't
					// have an empty key set
					...
					if (selector.keys().size() == 0 && wait == QUEUE_EMPTY) {
						...
						// selector.keys().size() + " keepRunning: " +
						// keepRunning);
						return;
					}
				} else {
			...
			if (needsSelect) {
				if (wait == QUEUE_EMPTY) {
					...
					// nothing to select immediately but there's something
					// pending so let's block that duration and wait.
			...
			try {
				if (key.isAcceptable()) {
					...
					try {
						...
						ckey = sc.register(selector.getSelector(),
								SelectionKey.OP_READ);
						ListenCallback serverHandler = (ListenCallback) key
								.attachment();
						AsyncNetworkSocket handler = new AsyncNetworkSocket();
						handler.attach(sc, (InetSocketAddress) sc.socket()
					...
					AsyncNetworkSocket handler = (AsyncNetworkSocket) key
							.attachment();
					...
					try {
						...
						newHandler.attach(sc, (InetSocketAddress) sc.socket()
								.getRemoteSocketAddress());
					...
					try {
						if (cancel.setComplete(newHandler))
							cancel.callback
									.onConnectCompleted(null, newHandler);
					} catch (Exception e) {
