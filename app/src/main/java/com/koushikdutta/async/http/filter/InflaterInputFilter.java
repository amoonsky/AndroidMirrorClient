...
public class InflaterInputFilter extends FilteredDataEmitter {
	...
	protected void report(Exception e) {
		...
		if (e != null && mInflater.getRemaining() > 0) {
			e = new DataRemainingException("data still remaining in inflater",
					e);
		}
		super.report(e);
	}
		...
		try {
			...
			while (bb.size() > 0) {
				...
				if (b.hasRemaining()) {
					...
					mInflater.setInput(b.array(),
							b.arrayOffset() + b.position(), b.remaining());
					do {
						int inflated = mInflater.inflate(output.array(),
