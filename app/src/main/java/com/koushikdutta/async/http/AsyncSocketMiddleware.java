...
public class AsyncSocketMiddleware extends SimpleMiddleware {
	...
	protected ConnectCallback wrapCallback(GetSocketData data, Uri uri,
			int port, boolean proxied, ConnectCallback callback) {
		return callback;
	}
	...
	String computeLookup(Uri uri, int port, String proxyHost, int proxyPort) {
		...
		return uri.getScheme() + "//" + uri.getHost() + ":" + port + "?proxy="
				+ proxy;
	}
	...
	public Cancellable getSocket(final GetSocketData data) {
		...
		final String lookup = computeLookup(uri, port,
				data.request.getProxyHost(), data.request.getProxyPort());
		...
		synchronized (AsyncSocketMiddleware.this) {
			...
			while (!info.sockets.isEmpty()) {
				...
				if (idleSocketHolder.idleTime + idleTimeoutMs < System
						.currentTimeMillis()) {
		...
		if (!connectAllAddresses || proxyHost != null
				|| data.request.getProxyHost() != null) {
			...
			if (proxied) {
				data.request.logv("Using proxy: " + unresolvedHost + ":"
						+ unresolvedPort);
			}
			return mClient.getServer()
					.connectSocket(
							unresolvedHost,
							unresolvedPort,
							wrapCallback(data, uri, port, proxied,
									data.connectCallback));
		}

		// try to connect to everything...
		data.request
				.logv("Resolving domain and connecting to all available addresses");
		return mClient.getServer().getAllByName(uri.getHost())
				.then(new TransformFuture<AsyncSocket, InetAddress[]>() {
					...
					protected void error(Exception e) {
						super.error(e);
						wrapCallback(data, uri, port, false,
								data.connectCallback).onConnectCompleted(e,
								null);
					}

					@Override
					protected void transform(final InetAddress[] result)
							throws Exception {
						Continuation keepTrying = new Continuation(
								new CompletedCallback() {
									@Override
									public void onCompleted(Exception ex) {
										// if it completed, that means that the
										// connection failed
										if (lastException == null)
											lastException = new ConnectionFailedException(
													"Unable to connect to remote address");
										if (setComplete(lastException)) {
											wrapCallback(data, uri, port,
													false, data.connectCallback)
													.onConnectCompleted(
															lastException, null);
										}
									}
						...
						for (final InetAddress address : result) {
							final String inetSockAddress = String.format(
									Locale.ENGLISH, "%s:%s", address, port);
							keepTrying.add(new ContinuationCallback() {
								@Override
								public void onContinue(
										Continuation continuation,
										final CompletedCallback next)
										throws Exception {
									data.request
											.logv("attempting connection to "
													+ inetSockAddress);
									mClient.getServer()
											.connectSocket(
																...
																public void onConnectCompleted(
																	...
																	if (isDone()) {
																		lastException = new Exception(
																				"internal error during connect to "
																						+ inetSockAddress);
																	...
																	if (isDone()
																			|| isCancelled()) {
																		data.request
																				.logd("Recycling extra socket leftover from cancelled operation");
																		idleSocket(socket);
																		recycleSocket(
																				socket,
																				data.request);
	...
	private void maybeCleanupConnectionInfo(String lookup) {
		...
		if (info.openCount == 0 && info.queue.isEmpty()
				&& info.sockets.isEmpty())
			connectionInfo.remove(lookup);
	}

	private void recycleSocket(final AsyncSocket socket,
			AsyncHttpRequest request) {
		if (socket == null)
			return;
		Uri uri = request.getUri();
		int port = getSchemePort(uri);
		final String lookup = computeLookup(uri, port, request.getProxyHost(),
	...
	private void nextConnection(AsyncHttpRequest request) {
		...
		String key = computeLookup(uri, port, request.getProxyHost(),
				request.getProxyPort());
		synchronized (AsyncSocketMiddleware.this) {
	...
	public void onResponseComplete(
			final OnResponseCompleteDataOnRequestSentData data) {
		...
		try {
			...
			if (!HttpUtil.isKeepAlive(data.response.protocol(),
					data.response.headers())
					|| !HttpUtil.isKeepAlive(Protocol.HTTP_1_1,
