...
public class SSLEngineSNIConfigurator implements AsyncSSLEngineConfigurator {
	private static class EngineHolder implements AsyncSSLEngineConfigurator {
		...
		public EngineHolder(Class engineClass) {
			try {
				peerHost = engineClass.getSuperclass().getDeclaredField(
						"peerHost");
				peerHost.setAccessible(true);

				peerPort = engineClass.getSuperclass().getDeclaredField(
						"peerPort");
		...
		public void configureEngine(SSLEngine engine,
				AsyncHttpClientMiddleware.GetSocketData data, String host,
				int port) {
			if (useSni == null)
				return;
			try {
	...
	@Override
	public void configureEngine(SSLEngine engine,
			AsyncHttpClientMiddleware.GetSocketData data, String host, int port) {
