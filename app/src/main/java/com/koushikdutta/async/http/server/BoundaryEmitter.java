...
public class BoundaryEmitter extends FilteredDataEmitter {
	 ...
	 * boundary=----------------------------bc3c801ac760
	 * 
	 * ------------------------------bc3c801ac760 Content-Disposition:
	 * form-data; name="my-file"; filename="foo" Content-Type:
	...
	public void onDataAvailable(DataEmitter emitter, ByteBufferList bb) {
		...
		for (int i = 0; i < buf.length; i++) {
			if (state >= 0) {
				...
				if (buf[i] == '\r') {
					...
					if (last != 0 || len != 0) {
						ByteBuffer b = ByteBufferList.obtain(len).put(buf,
								last, len);
						b.flip();
					...
					report(new MimeEncodingException(
							"Invalid multipart/form-data. Expected \r or -"));
					return;
				}
			} else if (state == -2) {
				if (buf[i] == '-') {
				...
				if (buf[i] == '\r') {
					state = -4;
					ByteBuffer b = ByteBufferList.obtain(
							i - last - boundary.length - 2).put(buf, last,
							i - last - boundary.length - 2);
					b.flip();
					ByteBufferList list = new ByteBufferList();
					list.add(b);
					super.onDataAvailable(this, list);
					...
					report(new MimeEncodingException(
							"Invalid multipart/form-data. Expected \r"));
					return;
				}
			} else if (state == -4) {
				if (buf[i] == '\n') {
		...
		if (last < buf.length) {
			// System.out.println("amount left at boundary: " + (buf.length -
			// last));
			// System.out.println("State: " + state);
			// System.out.println(state);
			int keep = Math.max(state, 0);
			ByteBuffer b = ByteBufferList.obtain(buf.length - last - keep).put(
					buf, last, buf.length - last - keep);
			b.flip();
			ByteBufferList list = new ByteBufferList();
			list.add(b);
			super.onDataAvailable(this, list);
