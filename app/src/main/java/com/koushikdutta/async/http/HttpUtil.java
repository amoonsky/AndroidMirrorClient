...
public class HttpUtil {
	public static AsyncHttpRequestBody getBody(DataEmitter emitter,
			CompletedCallback reporter, Headers headers) {
	...
	static class EndEmitter extends FilteredDataEmitter {
		...
		public static EndEmitter create(AsyncServer server, final Exception e) {
			...
			// don't need to worry about any race conditions with post and this
			// return value
	...
	public static DataEmitter getBodyDecoder(DataEmitter emitter,
			Protocol protocol, Headers headers, boolean server) {
		long _contentLength;
		try {
		...
		if (-1 != contentLength) {
			if (contentLength < 0) {
				EndEmitter ender = EndEmitter
						.create(emitter.getServer(),
			...
			ContentLengthFilter contentLengthWatcher = new ContentLengthFilter(
					contentLength);
			...
			if ((server || protocol == Protocol.HTTP_1_1)
					&& !"close".equalsIgnoreCase(headers.get("Connection"))) {
				// if this is the server, and the client has not indicated a
				// request body, the client is done
				EndEmitter ender = EndEmitter.create(emitter.getServer(), null);
				ender.setDataEmitter(emitter);
				emitter = ender;
				return emitter;
		...
		// conversely, if this is the client (http 1.0), and the server has not
		// indicated a request body, we do not report
