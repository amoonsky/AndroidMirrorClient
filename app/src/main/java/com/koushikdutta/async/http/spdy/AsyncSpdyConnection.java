...
public class AsyncSpdyConnection implements FrameReader.Handler {
	 ...
	 */
	public SpdySocket newStream(List<Header> requestHeaders, boolean out,
			boolean in) {
		return newStream(0, requestHeaders, out, in);
	}

	private SpdySocket newStream(int associatedStreamId,
			List<Header> requestHeaders, boolean out, boolean in) {
		...
		nextStreamId += 2;
		socket = new SpdySocket(streamId, outFinished, inFinished,
				requestHeaders);
		if (socket.isOpen()) {
		...
		try {
			if (associatedStreamId == 0) {
				writer.synStream(outFinished, inFinished, streamId,
	...
	void updateWindowRead(int length) {
		...
		if (totalWindowRead >= okHttpSettings
				.getInitialWindowSize(DEFAULT_INITIAL_WINDOW_SIZE) / 2) {
			try {
	...
	public class SpdySocket implements AsyncSocket {
		long bytesLeftInWriteWindow = AsyncSpdyConnection.this.peerSettings
				.getInitialWindowSize(Settings.DEFAULT_INITIAL_WINDOW_SIZE);
		...
		public SpdySocket(int id, boolean outFinished, boolean inFinished,
		...
		public void write(ByteBufferList bb) {
			int canWrite = (int) Math.min(bytesLeftInWriteWindow,
					AsyncSpdyConnection.this.bytesLeftInWriteWindow);
	...
	public AsyncSpdyConnection(AsyncSocket socket, Protocol protocol) {
		...
		// up the heap.
		if (client) {
			okHttpSettings.set(Settings.INITIAL_WINDOW_SIZE, 0,
	...
	public void sendConnectionPreface() throws IOException {
		...
		int windowSize = okHttpSettings
				.getInitialWindowSize(Settings.DEFAULT_INITIAL_WINDOW_SIZE);
		if (windowSize != Settings.DEFAULT_INITIAL_WINDOW_SIZE) {
			writer.windowUpdate(0, windowSize
	...
	private boolean pushedStream(int streamId) {
		return protocol == Protocol.HTTP_2 && streamId != 0
				&& (streamId & 1) == 0;
	}
	...
	public void headers(boolean outFinished, boolean inFinished, int streamId,
		...
		if (socket == null) {
			// The headers claim to be for an existing stream, but we don't have
			// one.
			...
			// If the stream ID is less than the last created ID, assume it's
			// already closed.
			if (streamId <= lastGoodStreamId)
				return;

			// If the stream ID is in the client's namespace, assume it's
			// already closed.
	...
	public void settings(boolean clearPrevious, Settings settings) {
		long delta = 0;
		int priorWriteWindowSize = peerSettings
				.getInitialWindowSize(DEFAULT_INITIAL_WINDOW_SIZE);
		if (clearPrevious)
			peerSettings.clear();
		...
		int peerInitialWindowSize = peerSettings
				.getInitialWindowSize(DEFAULT_INITIAL_WINDOW_SIZE);
		if (peerInitialWindowSize != -1
				&& peerInitialWindowSize != priorWriteWindowSize) {
	...
	private void writePing(boolean reply, int payload1, int payload2, Ping ping)
			throws IOException {
		if (ping != null)
			ping.send();
	...
	public void goAway(int lastGoodStreamId, ErrorCode errorCode,
		...
		for (Iterator<Map.Entry<Integer, SpdySocket>> i = sockets.entrySet()
				.iterator(); i.hasNext();) {
			Map.Entry<Integer, SpdySocket> entry = i.next();
			int streamId = entry.getKey();
			if (streamId > lastGoodStreamId
					&& entry.getValue().isLocallyInitiated()) {
				Util.end(entry.getValue(), new IOException(
						ErrorCode.REFUSED_STREAM.toString()));
				i.remove();
			}
		}
	}
	...
	@Override
	public void alternateService(int streamId, String origin,
			ByteString protocol, String host, int port, long maxAge) {
	}
