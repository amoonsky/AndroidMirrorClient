...
public class SocketIOClient extends EventEmitter {
	...
	public static Future<SocketIOClient> connect(final AsyncHttpClient client,
			String uri, final ConnectCallback callback) {
	...
	public static Future<SocketIOClient> connect(final AsyncHttpClient client,
			final SocketIORequest request, final ConnectCallback callback) {
		final SimpleFuture<SocketIOClient> ret = new SimpleFuture<SocketIOClient>();

		final SocketIOConnection connection = new SocketIOConnection(client,
				request);
		...
		connection.clients.add(new SocketIOClient(connection, "",
				wrappedCallback));
	...
	public void of(String endpoint, ConnectCallback connectCallback) {
		connection.connect(new SocketIOClient(connection, endpoint,
				connectCallback));
	}
