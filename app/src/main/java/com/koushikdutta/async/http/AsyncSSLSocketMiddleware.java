...
public class AsyncSSLSocketMiddleware extends AsyncSocketMiddleware {
	...
	public SSLContext getSSLContext() {
		return sslContext != null ? sslContext : AsyncSSLSocketWrapper
	...
	public void addEngineConfigurator(
	...
	protected SSLEngine createConfiguredSSLEngine(GetSocketData data,
	...
	protected void tryHandshake(AsyncSocket socket, GetSocketData data,
			final Uri uri, final int port, final ConnectCallback callback) {
	...
	protected ConnectCallback wrapCallback(final GetSocketData data,
		...
		return new ConnectCallback() {
			...
			public void onConnectCompleted(Exception ex,
				...
				// this SSL connection is proxied, must issue a CONNECT request
				// to the proxy server
				// http://stackoverflow.com/a/6594880/704837
				// some proxies also require 'Host' header, it should be safe to
				// provide it every time
				String connect = String.format(Locale.ENGLISH,
						"CONNECT %s:%s HTTP/1.1\r\nHost: %s\r\n\r\n",
						uri.getHost(), port, uri.getHost());
				data.request.logv("Proxying: " + connect);
				Util.writeAll(socket, connect.getBytes(),
								...
								liner.setLineCallback(new LineEmitter.StringCallback() {
									...
									public void onStringAvailable(String s) {
										...
										if (statusLine == null) {
											statusLine = s.trim();
											if (!statusLine
													.matches("HTTP/1.\\d 2\\d\\d .*")) { // connect
																							// response
																							// is
																							// allowed
																							// to
										...
										} else if (TextUtils.isEmpty(s.trim())) { // skip
																					// all
																					// headers,
																					// complete
																					// handshake
								...
								socket.setEndCallback(new CompletedCallback() {
									...
									public void onCompleted(Exception ex) {
										if (!socket.isOpen() && ex == null)
											ex = new IOException(
													"socket closed before proxy connect response");
										callback.onConnectCompleted(ex, socket);
									}
								});
							}
