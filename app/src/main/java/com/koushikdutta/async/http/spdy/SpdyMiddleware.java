...
public class SpdyMiddleware extends AsyncSSLSocketMiddleware {
	public SpdyMiddleware(AsyncHttpClient client) {
		...
		addEngineConfigurator(new AsyncSSLEngineConfigurator() {
			@Override
			public void configureEngine(SSLEngine engine, GetSocketData data,
	...
	private void configure(SSLEngine engine, GetSocketData data, String host,
			int port) {
		if (!initialized && spdyEnabled) {
			initialized = true;
			try {
				peerHost = engine.getClass().getSuperclass()
						.getDeclaredField("peerHost");
				peerPort = engine.getClass().getSuperclass()
						.getDeclaredField("peerPort");
				sslParameters = engine.getClass().getDeclaredField(
						"sslParameters");
				npnProtocols = sslParameters.getType().getDeclaredField(
						"npnProtocols");
				alpnProtocols = sslParameters.getType().getDeclaredField(
						"alpnProtocols");
				useSni = sslParameters.getType().getDeclaredField("useSni");
				sslNativePointer = engine.getClass().getDeclaredField(
						"sslNativePointer");
				String nativeCryptoName = sslParameters.getType().getPackage()
						.getName()
						+ ".NativeCrypto";
				nativeGetNpnNegotiatedProtocol = Class.forName(
						nativeCryptoName, true,
						sslParameters.getType().getClassLoader())
						.getDeclaredMethod("SSL_get_npn_negotiated_protocol",
								long.class);
				nativeGetAlpnNegotiatedProtocol = Class
						.forName(nativeCryptoName, true,
								sslParameters.getType().getClassLoader())
		...
		// TODO: figure out why POST does not work if sending content-length
		// header
		// see above regarding app engine comment as to why: drive requires
		// content-length
	...
	private static class SpdyConnectionWaiter extends
			MultiFuture<AsyncSpdyConnection> {
	...
	private void invokeConnect(String key, final ConnectCallback callback,
			Exception e, AsyncSSLSocket socket) {
	...
	@Override
	protected AsyncSSLSocketWrapper.HandshakeCallback createHandshakeCallback(
			final GetSocketData data, final ConnectCallback callback) {
		...
		return new AsyncSSLSocketWrapper.HandshakeCallback() {
			...
			public void onHandshakeCompleted(Exception e, AsyncSSLSocket socket) {
				...
				String protoString;
				try {
					long ptr = (Long) sslNativePointer.get(socket
							.getSSLEngine());
					byte[] proto = (byte[]) nativeGetAlpnNegotiatedProtocol
							.invoke(null, ptr);
					if (proto == null) {
				...
				final AsyncSpdyConnection connection = new AsyncSpdyConnection(
						socket, Protocol.get(protoString)) {
					boolean hasReceivedSettings;

					@Override
					public void settings(boolean clearPrevious,
							Settings settings) {
						...
						if (!hasReceivedSettings) {
							...
							if (waiter.originalCancellable.setComplete()) {
								data.request
										.logv("using new spdy connection for host: "
												+ data.request.getUri()
														.getHost());
	...
	private void newSocket(GetSocketData data,
			final AsyncSpdyConnection connection, final ConnectCallback callback) {
		...
		// request.getHeaders().set("Content-Length",
		...
		headers.add(new Header(Header.TARGET_PATH,
		...
		if (Protocol.SPDY_3 == connection.protocol) {
			...
			headers.add(new Header(Header.TARGET_AUTHORITY, host)); // Optional
																	// in HTTP/2
		} else {
			throw new AssertionError();
		}
		headers.add(new Header(Header.TARGET_SCHEME, request.getUri()
				.getScheme()));
		...
		request.logv("\n" + request);
		final AsyncSpdyConnection.SpdySocket spdy = connection.newStream(
				headers, requestBody != null, true);
	...
	@Override
	protected ConnectCallback wrapCallback(final GetSocketData data,
			final Uri uri, final int port, final boolean proxied,
			ConnectCallback callback) {
		final ConnectCallback superCallback = super.wrapCallback(data, uri,
				port, proxied, callback);
		final String key = data.state.get("spdykey");
		if (key == null)
		...
		return new ConnectCallback() {
			...
			public void onConnectCompleted(Exception ex, AsyncSocket socket) {
				// an exception here is an ssl or network exception... don't
				// rule spdy out yet, but
				// trigger the waiters
				if (ex != null) {
	...
	public Cancellable getSocket(final GetSocketData data) {
		...
		// can we use an existing connection to satisfy this, or do we need a
		// new one?
		...
		if (conn == null) {
			// no connection has ever been attempted (or previous one had a
			// network death), so attempt one
		...
		data.request.logv("waiting for potential spdy connection for host: "
				+ data.request.getUri().getHost());
		...
		conn.setCallback(new FutureCallback<AsyncSpdyConnection>() {
			...
			public void onCompleted(Exception e, AsyncSpdyConnection conn) {
				...
				data.request.logv("using existing spdy connection for host: "
						+ data.request.getUri().getHost());
				if (ret.setComplete())
	...
	public boolean exchangeHeaders(final OnExchangeHeaderData data) {
		...
		spdySocket.headers().then(new TransformFuture<Headers, List<Header>>() {
			...
			public void onCompleted(Exception e, Headers result) {
				...
				DataEmitter emitter = HttpUtil.getBodyDecoder(spdySocket,
						spdySocket.getConnection().protocol, result, false);
