...
final class HpackDraft08 {
	...
	private static final Header[] STATIC_HEADER_TABLE = new Header[] {
			...
			new Header("accept-language", ""), new Header("accept-ranges", ""),
			new Header("accept", ""),
			new Header("access-control-allow-origin", ""),
			new Header("age", ""), new Header("allow", ""),
			...
			new Header("content-range", ""), new Header("content-type", ""),
			new Header("cookie", ""), new Header("date", ""),
			new Header("etag", ""), new Header("expect", ""),
			new Header("expires", ""), new Header("from", ""),
			new Header("host", ""), new Header("if-match", ""),
			new Header("if-modified-since", ""),
			new Header("if-none-match", ""), new Header("if-range", ""),
			new Header("if-unmodified-since", ""),
			new Header("last-modified", ""), new Header("link", ""),
			new Header("location", ""), new Header("max-forwards", ""),
			new Header("proxy-authenticate", ""),
			new Header("proxy-authorization", ""), new Header("range", ""),
			new Header("referer", ""), new Header("refresh", ""),
			new Header("retry-after", ""), new Header("server", ""),
			new Header("set-cookie", ""),
			new Header("strict-transport-security", ""),
			new Header("transfer-encoding", ""), new Header("user-agent", ""),
			new Header("vary", ""), new Header("via", ""),
	...
	static final class Reader {
		...
		/**
		 * Set bit positions indicate {@code headerTable[pos]} should be
		 * emitted.
		 */
		...
		/**
		 * Set bit positions indicate {@code headerTable[pos]} was already
		 * emitted.
		 */
		...
		/**
		 * Called by the reader when the peer sent a new header table size
		 * setting. While this establishes the maximum header table size, the
		 ...
		 */
		void maxHeaderTableByteCountSetting(
		...
		private void adjustHeaderTableByteCount() {
			if (maxHeaderTableByteCount < headerTableByteCount) {
				if (maxHeaderTableByteCount == 0) {
					...
					evictToRecoverBytes(headerTableByteCount
							- maxHeaderTableByteCount);
				}
			}
		}
		...
		private int evictToRecoverBytes(int bytesToRecover) {
			...
			if (bytesToRecover > 0) {
				...
				for (int j = headerTable.length - 1; j >= nextHeaderIndex
						&& bytesToRecover > 0; j--) {
		...
		/**
		 * Read {@code byteCount} bytes of headers from the source stream into
		 * the set of emitted headers. This implementation does not propagate
		 * the never indexed flag of a header.
		 */
					...
					if ((b & 0x10) == 0x10) { // 0011NNNN
						if ((b & 0x0f) != 0)
							throw new IOException(
									"Invalid header table state change " + b);
						...
						if (maxHeaderTableByteCount < 0
							...
							throw new IOException(
									"Invalid header table byte count "
											+ maxHeaderTableByteCount);
						}
						adjustHeaderTableByteCount();
					}
		...
		/**
		 * Returns all headers emitted since they were last cleared, then clears
		 * the emitted headers.
		 */
		...
		private void readIndexedHeader(int index) throws IOException {
			if (isStaticHeader(index)) {
				...
				if (index > STATIC_HEADER_TABLE.length - 1) {
					throw new IOException("Header index too large "
							+ (index + 1));
				}
				...
				if (!referencedHeaders.get(headerTableIndex)) { // When
																// re-referencing,
																// emit
																// immediately.
		...
		private void readLiteralHeaderWithIncrementalIndexingIndexedName(
		...
		private void insertIntoHeaderTable(int index, Header entry) {
			...
			int bytesToRecover = (headerTableByteCount + delta)
					- maxHeaderTableByteCount;
			int entriesEvicted = evictToRecoverBytes(bytesToRecover);

			if (index == -1) { // Adding a value to the header table.
				if (headerCount + 1 > headerTable.length) { // Need to grow the
															// header table.
					Header[] doubled = new Header[headerTable.length * 2];
					System.arraycopy(headerTable, 0, doubled,
							headerTable.length, headerTable.length);
					if (doubled.length == 64) {
						referencedHeaders = ((BitArray.FixedCapacity) referencedHeaders)
								.toVariableCapacity();
						emittedReferencedHeaders = ((BitArray.FixedCapacity) emittedReferencedHeaders)
								.toVariableCapacity();
					}
		...
		ByteString readByteString() throws IOException {
			...
			if (huffmanDecode) {
				return ByteString.of(Huffman.get().decode(
	...
	private static Map<ByteString, Integer> nameToFirstIndex() {
		Map<ByteString, Integer> result = new LinkedHashMap<ByteString, Integer>(
				STATIC_HEADER_TABLE.length);
	...
	static final class Writer {
		...
		ByteBufferList writeHeaders(List<Header> headerBlock)
				throws IOException {
			...
			for (int i = 0, size = headerBlock.size(); i < size; i++) {
				...
				if (staticIndex != null) {
					...
					current.put((byte) 0x00); // Literal Header without Indexing
												// - New Name.
		...
		void writeByteString(ByteBuffer out, ByteString data)
				throws IOException {
	...
	/**
	 * An HTTP/2 response cannot contain uppercase header characters and must be
