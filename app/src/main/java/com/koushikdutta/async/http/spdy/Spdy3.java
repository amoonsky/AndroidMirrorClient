...
final class Spdy3 implements Variant {
	...
	static final class Reader implements FrameReader {
		...
		private final DataCallback onFrame = new DataCallback() {
			...
			public void onDataAvailable(DataEmitter emitter, ByteBufferList bb) {
				...
				if (!control) {
					...
					if (length == 0) {
						// zero length packet, immediately trigger the data
						// parsing
						// fixes the hanging response portion of
						// https://github.com/koush/ion/issues/443#issuecomment-67729152
		...
		private void readSynStream(ByteBufferList source, int flags, int length)
				throws IOException {
			int w1 = source.getInt();
			int w2 = source.getInt();
			int streamId = w1 & 0x7fffffff;
			int associatedStreamId = w2 & 0x7fffffff;
			source.getShort(); // int priority = (s3 & 0xe000) >>> 13; int slot
								// = s3 & 0xff;
			List<Header> headerBlock = headerReader.readHeader(source,
					length - 10);

			boolean inFinished = (flags & FLAG_FIN) != 0;
			boolean outFinished = (flags & FLAG_UNIDIRECTIONAL) != 0;
			handler.headers(outFinished, inFinished, streamId,
					associatedStreamId, headerBlock,
					HeadersMode.SPDY_SYN_STREAM);
		}

		private void readSynReply(ByteBufferList source, int flags, int length)
				throws IOException {
			int w1 = source.getInt();
			int streamId = w1 & 0x7fffffff;
			List<Header> headerBlock = headerReader.readHeader(source,
					length - 4);
			boolean inFinished = (flags & FLAG_FIN) != 0;
			handler.headers(false, inFinished, streamId, -1, headerBlock,
					HeadersMode.SPDY_REPLY);
		}

		private void readRstStream(ByteBufferList source, int flags, int length)
				throws IOException {
			if (length != 8)
		...
		private void readHeaders(ByteBufferList source, int flags, int length)
				throws IOException {
			int w1 = source.getInt();
			int streamId = w1 & 0x7fffffff;
			List<Header> headerBlock = headerReader.readHeader(source,
					length - 4);
			handler.headers(false, false, streamId, -1, headerBlock,
					HeadersMode.SPDY_HEADERS);
		}

		private void readWindowUpdate(ByteBufferList source, int flags,
		...
		private void readPing(ByteBufferList source, int flags, int length)
				throws IOException {
			if (length != 4)
		...
		private void readGoAway(ByteBufferList source, int flags, int length)
				throws IOException {
			if (length != 8)
		...
		private void readSettings(ByteBufferList source, int flags, int length)
				throws IOException {
			int numberOfEntries = source.getInt();
			if (length != 4 + 8 * numberOfEntries) {
				throw ioException("TYPE_SETTINGS length: %d != 4 + 8 * %d",
	...
	static final class Writer implements FrameWriter {
		...
		@Override
		public void pushPromise(int streamId, int promisedStreamId,
		...
		public synchronized void synStream(boolean outFinished,
			...
			int flags = (outFinished ? FLAG_FIN : 0)
					| (inFinished ? FLAG_UNIDIRECTIONAL : 0);
		...
		void sendDataFrame(int streamId, int flags, ByteBufferList buffer)
			...
			if (byteCount > 0xffffffL) {
				throw new IllegalArgumentException(
						"FRAME_TOO_LARGE max size is 16Mib: " + byteCount);
			}
			ByteBuffer sink = ByteBufferList.obtain(256).order(
					ByteOrder.BIG_ENDIAN);
			sink.putInt(streamId & 0x7fffffff);
		...
		private ByteBufferList writeNameValueBlockToBuffer(
				List<Header> headerBlock) throws IOException {
			if (headerBlockList.hasRemaining())
				throw new IllegalStateException();
			ByteBuffer headerBlockOut = ByteBufferList.obtain(8192).order(
					ByteOrder.BIG_ENDIAN);
			...
			for (int i = 0, size = headerBlock.size(); i < size; i++) {
				...
				if (headerBlockOut.remaining() < headerBlockOut.capacity() / 2) {
					ByteBuffer newOut = ByteBufferList.obtain(
							headerBlockOut.capacity() * 2).order(
							ByteOrder.BIG_ENDIAN);
					headerBlockOut.flip();
			...
			headerBlockOut.flip();
			deflater.setInput(headerBlockOut.array(), 0,
					headerBlockOut.remaining());
			while (!deflater.needsInput()) {
				ByteBuffer deflated = ByteBufferList.obtain(
						headerBlockOut.capacity()).order(ByteOrder.BIG_ENDIAN);
				int read = deflater.deflate(deflated.array(), 0,
						deflated.capacity(), Deflater.SYNC_FLUSH);
				deflated.limit(read);
		...
		public synchronized void goAway(int lastGoodStreamId,
				ErrorCode errorCode, byte[] ignored) throws IOException {
			if (closed)
				throw new IOException("closed");
			if (errorCode.spdyGoAwayCode == -1) {
				throw new IllegalArgumentException(
						"errorCode.spdyGoAwayCode == -1");
			}
		...
		public synchronized void windowUpdate(int streamId, long increment)
			...
			if (increment == 0 || increment > 0x7fffffffL) {
				throw new IllegalArgumentException(
						"windowSizeIncrement must be between 1 and 0x7fffffff: "
								+ increment);
			}
