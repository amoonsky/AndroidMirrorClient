...
public class ChunkedInputFilter extends FilteredDataEmitter {
	...
	private static enum State {
		CHUNK_LEN, CHUNK_LEN_CR, CHUNK_LEN_CRLF, CHUNK, CHUNK_CR, CHUNK_CRLF, COMPLETE
	}

	private boolean checkByte(char b, char value) {
		if (b != value) {
			report(new ChunkedDataException(value + " was expected, got "
					+ (char) b));
			return false;
		}
		return true;
	...
	protected void report(Exception e) {
		if (e == null && mState != State.COMPLETE)
			e = new ChunkedDataException(
					"chunked input ended before final chunk");
		super.report(e);
	}
		...
		try {
			while (bb.remaining() > 0) {
				...
				case CHUNK_LEN:
					...
					if (c == '\r') {
						...
						if (c >= 'a' && c <= 'f')
							...
							report(new ChunkedDataException(
									"invalid chunk length: " + c));
							return;
						}
					}
				...
				case COMPLETE:
					assert false;
					// Exception fail = new
					// Exception("Continued receiving data after chunk complete");
					// report(fail);
					return;
				}
			}
