...
public class AsyncHttpRequest {
	public RequestLine getRequestLine() {
		return new RequestLine() {
			...
			public String toString() {
				if (proxyHost != null)
					return String.format(Locale.ENGLISH, "%s %s HTTP/1.1",
							mMethod, AsyncHttpRequest.this.getUri());
				...
				return String.format(Locale.ENGLISH, "%s %s HTTP/1.1", mMethod,
						path);
			}
		};
	}
	...
	protected static String getDefaultUserAgent() {
		...
		return agent != null ? agent : ("Java" + System
	...
	public AsyncHttpRequest setMethod(String method) {
		if (getClass() != AsyncHttpRequest.class)
			throw new UnsupportedOperationException(
					"can't change method on a subclass of AsyncHttpRequest");
		mMethod = method;
		return this;
	}
	...
	private String getLogMessage(String message) {
		...
		if (executionTime != 0)
			...
			elapsed = 0;
		return String.format(Locale.ENGLISH, "(%d ms) %s: %s", elapsed,
				getUri(), message);
	}
