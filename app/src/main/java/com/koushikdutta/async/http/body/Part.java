...
public class Part {
	...
	public Part(Headers headers) {
		mHeaders = headers;
		mContentDisposition = Multimap.parseSemicolonDelimited(mHeaders
	...
	public Part(String name, long length, List<NameValuePair> contentDisposition) {
		...
		StringBuilder builder = new StringBuilder(String.format(Locale.ENGLISH,
				"form-data; name=\"%s\"", name));
		if (contentDisposition != null) {
			for (NameValuePair pair : contentDisposition) {
				builder.append(String.format(Locale.ENGLISH, "; %s=\"%s\"",
