...
/**
 * Settings describe characteristics of the sending peer, which are used by the
 * receiving peer. Settings are
 * {@link com.koushikdutta.async.http.spdy.okhttp.internal.spdy.SpdyConnection
 * connection} scoped.
 */
final class Settings {
	...
	/**
	 * Sent by servers only. The peer requests this setting persisted for future
	 * connections.
	 */
	static final int PERSIST_VALUE = 0x1;
	/**
	 * Sent by clients only. The client is reminding the server of a persisted
	 * value.
	 */
	...
	/**
	 * HTTP/2: Size in bytes of the table used to decode the sender's header
	 * blocks.
	 */
	...
	/**
	 * spdy/3: Sender's estimate of millis between sending a request and
	...
	/**
	 * If set, flow control is disabled for streams directed to the sender of
	 * these settings.
	 */
	...
	int getClientCertificateVectorSize(int defaultValue) {
		...
		return (bit & set) != 0 ? values[CLIENT_CERTIFICATE_VECTOR_SIZE]
				: defaultValue;
	}
