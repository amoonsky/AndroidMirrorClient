...
public class HttpTransportMiddleware extends SimpleMiddleware {
	...
	public boolean exchangeHeaders(final OnExchangeHeaderData data) {
		...
		// try to get the request body in the same packet as the request
		// headers... if it will fit
		// in the max MTU (1540 or whatever).
		final boolean waitForBody = requestBody != null
				&& requestBody.length() >= 0 && requestBody.length() < 1024;
		...
		Util.writeAll(headerSink, rs.getBytes(), new CompletedCallback() {
			...
			public void onCompleted(Exception ex) {
				...
				// flush headers and any request body that was written by the
				// callback
				if (bsink != null) {
				...
				try {
					...
					if (statusLine == null) {
						...
						if (socket == null)
							return;
						DataEmitter emitter;
						// HEAD requests must not return any data. They still
						// may
						// return content length, etc, which will confuse the
						// body decoder
						if (AsyncHttpHead.METHOD.equalsIgnoreCase(data.request
								.getMethod())) {
							emitter = HttpUtil.EndEmitter.create(
									socket.getServer(), null);
						} else {
							emitter = HttpUtil.getBodyDecoder(socket,
									Protocol.get(protocol), mRawHeaders, false);
						}
