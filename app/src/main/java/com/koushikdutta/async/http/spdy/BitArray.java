...
interface BitArray {
		...
		private static int checkInput(int index) {
			if (index < 0 || index > 63) {
				throw new IllegalArgumentException(format(Locale.ENGLISH,
						"input must be between 0 and 63: %s", index));
			}
			return index;
		}
	}
	...
	public final class VariableCapacity implements BitArray {
		...
		// Start offset which allows for cheap shifting. Data is always kept on
		// 64-bit bounds but we
		// offset the outward facing index to support shifts without having to
		// move the underlying bits.
		...
		private static int checkInput(int index) {
			if (index < 0) {
				throw new IllegalArgumentException(format(Locale.ENGLISH,
						"input must be a positive number: %s", index));
			}
			return index;
		}
	}
