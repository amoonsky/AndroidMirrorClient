...
public class WebSocketImpl implements WebSocket {
		...
		mParser = new HybiParser(mSocket) {
			...
			protected void onMessage(String payload) {
				if (WebSocketImpl.this.mStringCallback != null)
					WebSocketImpl.this.mStringCallback
							.onStringAvailable(payload);
			}
	...
	public WebSocketImpl(AsyncHttpServerRequest request,
		...
		// match the protocol (sanity checking and enforcement is done in the
		// caller)
		if (!TextUtils.isEmpty(protocol))
			response.getHeaders().set("Sec-WebSocket-Protocol", protocol);
		// if (origin != null)
		// response.getHeaders().getHeaders().set("Access-Control-Allow-Origin",
		// "http://" + origin);
		response.writeHead();
	...
	public static void addWebSocketUpgradeHeaders(AsyncHttpRequest req,
		...
		final String key = Base64.encodeToString(
				toByteArray(UUID.randomUUID()), Base64.NO_WRAP);
	...
	public static WebSocket finishHandshake(Headers requestHeaders,
	...
	public void ping(String string) {
		mSink.write(new ByteBufferList(ByteBuffer.wrap(mParser
				.pingFrame(string))));
	}
