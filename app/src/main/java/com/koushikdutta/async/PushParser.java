...
public class PushParser implements DataCallback {
	...
	static abstract class Waiter {
		...
		/**
		 * Consumes received data, and/or returns next waiter to continue
		 * reading instead of this waiter.
		 * 
		 * @param bb
		 *            received data, bb.remaining >= length
		 * @return - a waiter that should continue reading right away, or null
		 *         if this waiter is finished
		 */
		public abstract Waiter onDataAvailable(DataEmitter emitter,
				ByteBufferList bb);
	}
	...
	static class ByteBufferListWaiter extends Waiter {
		...
		public ByteBufferListWaiter(int length,
				ParseCallback<ByteBufferList> callback) {
			super(length);
			if (length <= 0)
				throw new IllegalArgumentException("length should be > 0");
	...
	public PushParser readByteArray(int length) {
		return (length == -1) ? readLenByteArray() : readByteArray(length,
				byteArrayArgCallback);
	}
	...
	public PushParser readByteBufferList(int length) {
		return (length == -1) ? readLenByteBufferList() : readByteBufferList(
				length, byteBufferListArgCallback);
	}
	...
	public void onDataAvailable(DataEmitter emitter, ByteBufferList bb) {
		...
		while (mWaiting.size() > 0
				&& pending.remaining() >= mWaiting.peek().length) {
			pending.order(order);
			Waiter next = mWaiting.poll().onDataAvailable(emitter, pending);
			if (next != null)
				mWaiting.addFirst(next);
		}
		if (mWaiting.size() == 0)
			pending.get(bb);
	}
	...
	static Method getTap(TapCallback callback) {
		...
		// null !=
		// "AndroidAsync: tap callback could not be found. Proguard? Use this in your proguard config:\n"
		// + fail;
