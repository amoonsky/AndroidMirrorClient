...
public class SimpleFuture<T> extends SimpleCancellable implements
		DependentFuture<T> {
	...
	private FutureCallback<T> handleCompleteLocked() {
		...
		// can't simply call this.callback.onCompleted directly outside the
		// block,
		// because that may result in a race condition where the callback
		// changes once leaving
		// the block.
