...
public class ByteBufferList {
	...
	private ByteBuffer read(int count) {
		if (remaining() < count)
			throw new IllegalArgumentException("count : " + remaining() + "/"
					+ count);
	...
	public static void reclaim(ByteBuffer b) {
		...
		synchronized (LOCK) {
			while (currentSize > MAX_SIZE && r.size() > 0
					&& r.peek().capacity() < b.capacity()) {
	...
	public static void writeOutputStream(OutputStream out, ByteBuffer b)
			throws IOException {
		byte[] bytes;
		int offset;
		int length;
