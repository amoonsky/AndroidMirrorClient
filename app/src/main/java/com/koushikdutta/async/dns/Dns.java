...
public class Dns {
	...
	public static Cancellable multicastLookup(AsyncServer server, String host,
			FutureCallback<DnsResponse> callback) {
		return lookup(server, host, true, callback);
	}

	public static Cancellable multicastLookup(String host,
			FutureCallback<DnsResponse> callback) {
		return multicastLookup(AsyncServer.getDefault(), host, callback);
	}

	public static Future<DnsResponse> lookup(AsyncServer server, String host,
			final boolean multicast, final FutureCallback<DnsResponse> callback) {
		ByteBuffer packet = ByteBufferList.obtain(1024).order(
		...
		try {
			...
			if (!multicast) {
				dgram = server.connectDatagram(new InetSocketAddress("8.8.8.8",
						53));
			} else {
				// System.out.println("multicast dns...");
				dgram = AsyncServer.getDefault().openDatagram(
				...
				Method method = impl.getClass().getDeclaredMethod("join",
						InetAddress.class);
