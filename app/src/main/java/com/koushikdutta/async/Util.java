...
public class Util {
	...
	public static void emitAllData(DataEmitter emitter, ByteBufferList list) {
		...
		while (!emitter.isPaused()
				&& (handler = emitter.getDataCallback()) != null
				&& (remaining = list.remaining()) > 0) {
			handler.onDataAvailable(emitter, list);
			if (remaining == list.remaining()
				...
				// call byteBufferList.recycle() or read all the data to prevent
				// this assertion.
				// this is nice to have, as it identifies protocol or parsing
				// errors.
				...
				throw new RuntimeException(
						"mDataHandler failed to consume data, yet remains the mDataHandler.");
			}
		}
	...
	public static void pump(final InputStream is, final DataSink ds,
			final CompletedCallback callback) {
		pump(is, Integer.MAX_VALUE, ds, callback);
	}

	public static void pump(final InputStream is, final long max,
			final DataSink ds, final CompletedCallback callback) {
				...
				try {
					do {
						if (!pending.hasRemaining()) {
							...
							long toRead = Math.min(max - totalRead,
									b.capacity());
	...
	public static void pump(final File file, final DataSink ds,
			final CompletedCallback callback) {
		try {
	...
	public static <T extends AsyncSocket> T getWrappedSocket(
			AsyncSocket socket, Class<T> wrappedClass) {
